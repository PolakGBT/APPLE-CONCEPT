...
function draw(){
    ...
    for(var i=0;i<nbr;i++){
      tab[i] = i;
      tab[i] = [0,0,(Math.random()*(0.02 -0.01)+0.01),initM()];
      ctx.beginPath();
      ctx.arc(Xini(tab[i][0],tab[i][1]), Yini(tab[i][0],tab[i][1]), 0.75, 0, Math.PI * 2, true);
      ctx.fill();
    }
    ...
    function animate() {
      ...
      for(var w = 0;w<nbr;w++){
          ...
          rayonn = prop[1];
          ctx.beginPath();
          ctx.arc(Xini(anglee,rayonn), Yini(anglee,rayonn), 0.75, 0, Math.PI * 2, true);
          ctx.fill();
          tab[w][0] = anglee;
          tab[w][1] = rayonn;
          tab[w][3] = prop[2];
  ...
  function begin(a,r){
    r=Math.random()*(120-103)+103;
      if(a == 0){
        ...
        if(prob > 0.1){
          a = Math.random()*(3.14 -0.01)+0.01;
        }
        if(prob <= 0.9){
          a = 0 - (Math.random()*(3.14 -0.01)+0.01);
        }
        return[a,r];
      }
